...
- **八种武器** — 主炮 5 级进化（单发→双发→三向→五向→穿透） + 7 种副武器，最多携带 4 种：`Homing Missile`（追踪弹）、`Orbit Drone`（环绕僚机）、`Pulse Laser`（持续光束）、`Chain Bolt`（闪电链跳）、`Void Rift`（部署脉冲伤害场）、`Wave Cannon`（正弦波弹丸扫荡）、`Reflector`（弹丸碰壁反弹）。副武器 ≥3 个时有递减收益。
...
- **精英与 Boss** — 精英词缀 `Armored / Berserk / Dasher`，每章 Boss 从 `Frenzied / Bulwark / Summoner / StormCore / Phantom / Hydra` 池中随机抽取，各有独立的预警和攻击模式。
